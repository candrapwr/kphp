$(function () {
    ...
    $('.datatables-basic').DataTable({
        ajax: $('body').attr('data-asset-path') + 'master/bdh_all',
        columns: [{
                data: 'bdh_id'
            },
            {
...
function simpan(mode) {
    ...
    $.ajax({
        url: $('body').attr('data-asset-path') + 'master/bdh_add',
        type: 'post',
        dataType: 'json',
...
function hapus(dataId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: $('body').attr('data-asset-path') + 'master/bdh_del',
                type: 'post',
                dataType: 'json',
                data: {
                    dataId: dataId,
...
function edit(dataId) {
    $.ajax({
        url: $('body').attr('data-asset-path') + 'master/bdh_edit',
        type: 'post',
        dataType: 'json',
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        data: {
        ...
        success: function (response, textStatus, xhr) {
            console.log(response.data.bdh_id);
            $('input[name="bdh_name_e"]').val(response.data.bdh_name);
            $('input[name="dataId"]').val(response.data.bdh_id);
            $('#kph_id_e').val(response.data.kph_id);
